...
# You can find instructions in the course materials (wiki) and in the instructor
# notes.
...
# will need to process the data file in a way other than using split(","). To do
# this,
...
#
# In this exercise, we are interested in the field 'body' (which is the 5th
# field,
# line[4]). The objective is to count the number of forum nodes where 'body'
# either
# contains none of the three punctuation marks: period ('.'), exclamation point
# ('!'),
# question mark ('?'), or else 'body' contains exactly one such punctuation mark
# as the
# last character. There is no need to parse the HTML inside 'body'. Also, do not
# pay
...
def mapper():
    ...
    for line in reader:
        ...
        body = line[4]
        puncts = ["?", ".", "!"]
        has_no_punct = (body.find(".") == -1) and (body.find("!") == -1) and (body.find("?") == -1)
        text = body[:-1]
        flags = [text.find(p) > 0 for p in puncts]
        
        if has_no_punct:
            count += 1
            writer.writerow(line)
        
        if (body[-1] in puncts) and (sum(flags) == 0):
            count += 1
            writer.writerow(line)
